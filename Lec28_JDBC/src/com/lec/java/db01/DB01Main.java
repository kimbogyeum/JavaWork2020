...
//			sql_insert="INSERT INTO test_member VALUES("+ no+", '"+name+"', sysdate)";
			...
			System.out.println(cnt+"개row(행)이 INSERT 됨");
			...
			rs=stmt.executeQuery(sql_select); //'SELECT' 및 기타쿼리의 경우 executeQuery() 로 실행
										   //리턴값은 ResultSet 객체 
			
			//7. ResultSet에 담겨있는 record 확인
			System.out.println();
			...
			System.out.println();
			while(rs.next()) { //next()레코드 하나 추출하고 true리턴, 더이상 뽑아낼 레코드 없으면 FALSE 리턴
				String no=rs.getString(1); //select문에 썼던 query index 순서대로 1,2,3 (0부터 시작이 아니라는것이 핵심~!)
