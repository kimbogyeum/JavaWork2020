...
//		for (int i = 0; i < dataUrls.length; i++) app.download2(dataUrls[i]);
	...
	// 장점: 파일사이즈, 타입, 이름 등을 미리 알수 있다.-> 버퍼를 파일사이즈에 맞추면 다운로드 시간이 적게걸리지
	public void download2(String srcUrl) throws IOException {
		...
		//다운로드 받는 파일 이름 확인 가능,브라우저에서 다운받을때도 "Content-Disposition"쓴다.
		...
		byte[] buf=new byte[contentLength];// 다운로드 받을 파일크기만큼의 버퍼준비!
		int byteImg;
	...
	public void download3(String srcUrl) throws IOException {
		...
		System.out.println("다운로드:"+srcUrl+"->"+dstFile);
		System.out.println("\t"+imgData.getWidth()+"x"+imgData.getHeight());//이런정보까지 알 수 있는게 ImageIO를 쓰는 이유
	}//end download3
