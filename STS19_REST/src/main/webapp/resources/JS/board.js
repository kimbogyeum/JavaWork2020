...
$(document).ready(function(){
	...
	$("#viewUpdate").click(function(){
		setPopup("update"); //update모드로 popup창 바꾸기
	});
		...
		, success : function(data, status){
			if(status == "success"){
				...
				if(updateList(data)){
					...
					//업데이트 된 list에 필요한 이벤트 가동
					addViewEvent();
					//★만약 위 코드를 $(document).ready()에 두면 동작 안한다.왜냐하면 아직 글 문서 목록이 로딩안된상태이기 때문
				}
			}			
		}
	});
...
//- 제목(subject) 클릭하면 view 팝업화면 뜰 수 있게하기
	...
	$.ajax({
		...
		success:function(data, status){
			if(status=="success"){
				if(data.status=="OK"){
					alert("DELETE 성공"+data.count+"개");
					loadPage(window.page);//삭제되면 현재 페이지 로딩, window 빼도됨
				}else{
					alert("DELETE 실패"+data.message);
					return false;
				}
...
function chkUpdate(){
	...
	//ex) name=aaa&subject=bbb&content=ccc&uid1230
	var data=$("#frmWrite").serialize(); //string 형태
		...
		success: function(data,status){
			if(status=="success"){
				if(data.status=="OK"){
					...
					loadPage(window.page);// 업데이트 후 현재 페이지 리로딩
				}else{
