...
public class Operator09Main {
	...
	public static void main(String[] args) {
		...
		byte b1=10, b2=20;
//		byte b3= b1+b2; 에러남 -> byte+byte=>int
...
//		char ch3=ch1+ch2; //char+char -> int
		char ch3=(char)(ch1+ch2); //명시적 형변환 해주기 
...
//		int n4=n1+l1; //int+long->long
		...
		f3=f1+n1;
		f3=f1+b1;
		//float와 그 이하 잡것들의 합은 다 float가 나온다. 
//		f3=f1+d1;//float+double--> double
		
//		int 이하 끼리 연산결과 무조건 int 이기 떄문에 주의 !!
		...
		int sum=0;
		double d4=1.2;
//		sum=sum+d4; //int+double->double인데 sum이 int니까 거기에 대입하려니 오류가 나지
		sum +=d4; //에러가 안난다고?
		System.out.println(sum);
		//sum +=d4 복합대입연산자는 우리가 아는 sum = sum+d4가 아니라 
		//sum=(int)(sum+d4)이므로 동작한다. 
