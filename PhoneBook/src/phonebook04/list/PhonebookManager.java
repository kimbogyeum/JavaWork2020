...
// CONTROLLER 객체: sysout쓰지 않는다. 그거는 철저히 view가 해야한다. 
...
public class PhonebookManager implements Pb {
	...
	public PhonebookModel[] selectAll() {
		PhonebookModel[] pm=new PhonebookModel[pbList.size()];
		for (int i = 0; i <pbList.size(); i++) {
			pm[i]=pbList.get(i);
		}
		return pm;
//		pbList.toArray(new PhonebookModel[pbList.size()]);//이렇게 하는 방법도 있음: toArray: 
	}
